  ...
  <div class="lokasi-klinik">
    <div class="site-bg lazy">
      ...
      <div class="row judul-back">
        <h1 class="title-lokasi-home col-md-4 col-xs-12">Promos Rs . <?= strtoupper($rs) ?></h1>
      </div>
          ...
          ?>
              <div class="row">
                <div class="col-md-5 magazine-left">
                  <a target="_blank" href="<?= base_url(); ?><?= $rs ?>/promo/<?= $value->link ?>">
                ...
                <div class="col-md-7 magazine-right">
                  <div class="no-edisi-m content-gray"><a href="<?= base_url(); ?><?= $rs ?>/promo/<?= $value->link ?>" target="_blank" class="edisi"><b><?= ($value->updated_datetime); ?></b></a></div>
                  <h3><?= $value->title ?></h3>
                  <div class="desc-m content-gray"><?= strip_tags(substr($value->content, 0, 300)) . '....'; ?></div>
                  <a class="btn btn-border-pink button-detail-rp" target="_blank" href="<?= base_url(); ?><?= $rs ?>/promo/<?= $value->link ?>">Baca</a>
...
<script>
  function singleArticlePromo(article_id, article_category_id, id_rumahsakit) {
    setTimeout(function() {
      $.ajax({
        url: base_url + 'front/single_article_promo',
        dataType: 'html',
        data: {
          a_id: article_id,
        ...
        success: function(html) {
          $('#promo').hide('slow').replaceWith(html);
