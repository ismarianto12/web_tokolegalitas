...
<div class="nav-tabs-custom">
    <ul class="nav nav-tabs pull-right">
        <li class="active">
            <a data-toggle="tab" href="#video-table-tab" title="Table View">
                <i class="fa fa-table"></i>
            </a>
        </li>
        <li class="">
            <a data-toggle="tab" href="#video-form-tab" title="Form View">
                <i class="fa fa-edit"></i>
            </a>
        </li>
        <li class="pull-left header"><i class="fa fa-file-text"></i> Gallery Video </li>
        <div id="loading"></div>
    </ul>
    <div class="tab-content">
        <div id="video-table-tab" class="tab-pane fade active in">
            <table id="table-video" class="table table-bordered table-striped table-hover table-condensed">
                <thead>
                    <tr>
                        <th>Title</th>
                        <th>Publish</th>
        ...
        <div id="video-form-tab" class="tab-pane fade">
            <form class="form-horizontal" role="form" id="video-form">
                ...
                <div class="form-group">
                    <label for="content-input" class="col-md-3 control-label">Link</label>
                    <div class="col-md-6">
                        <input type="text" class="form-control" id="content-input" name="content-input" value="<?php echo $content;?>">
                    </div>
                </div>
                <div class="form-group" style="display: none;">
                    ...
                    <div class="col-md-6">
                        <select class="form-control"  name="category-input" id="category-input">
                            <?php
                            $cat = $this->model->getList(array('table' => 'article_category', 'where' => array('is_active' => 1,"is_video"=>1)));
                            if ($cat) {
                                foreach ($cat as $row) {
               ...
               <?= parsing_rs_select()  ?>
                ...
                <div class="form-group">
                    <div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-4">
                        ...
                        <button type="reset" class="btn btn-default" onclick="setActiveTab('video-table-tab');"><i class="fa fa-undo"></i> Cancel</button>
                    </div>
                </div>
            </form>
        </div>
...
<script>
    $(document).ready(function () {
        ...
        if($param != null) {
            echo 'getData("'. $param .'");';
            echo 'setActiveTab("video-form-tab");';
            echo '$("#div-foto").show();';
        }
        ?>
        ...
        $('#file_upload').on('filebatchuploadcomplete', function(event, files, extra) {
            loadContent(base_url + "view/_gallery_video_form/<?php echo $param;?>");
        });
    });

    function newForm() {
        loadContent(base_url + "view/_gallery_video_form", function () {
            setActiveTab("video-form-tab");
        });
    }

    function getArticle() {
        if ($.fn.dataTable.isDataTable('#table-video')) {
            tableArticle = $('#table-video').DataTable();
        } else {
            tableArticle = $('#table-video').DataTable({
                "ajax": base_url + 'objects/article/article_category_id/4',
    ...
    function utilsArticle() {
        $("#table-video .editBtn").on("click",function() {
            loadContent(base_url + 'view/_gallery_video_form/' + $(this).attr('href').substring(1));
        });

        $("#table-video .removeBtn").on("click",function() {
            ...
            success: function(json) {
                if (json.data.code === 0) {
                    ...
                    if (json.data.object.img !== null) {
                        $("#foto-div").html('<img src="<?php echo base_url();?>assets/image/article/'+json.data.object.img+'" class="img img-thumbnail img-small">');
                    }
                }
            }
        });
    ...
    function refreshTable(){
        tableArticle.ajax.url(base_url + '/objects/article/article_category_id/4').load();
    }
